...
function isDateInPeriod(date, period) {
  ...
  return dateMain >= startDate && dateMain <= endDate;
}
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hour12}:${minutes}:${seconds} ${amap}`;
}
...
function getWeekNumberByDate(date) {
  ...
  weekStart.setDate(jan4.getDate() - (jan4IsoDay - 1));

  const yearStart = new Date(target.getFullYear(), 0, 1);
  yearStart.setHours(0, 0, 0, 0);

  const startDay = yearStart.getDay();
  const isoStartDay = startDay === 0 ? 7 : startDay;

  const firstThursday = new Date(yearStart);
  firstThursday.setDate(yearStart.getDate() + (4 - isoStartDay));

  const diffInMs = target - weekStart;
...
function getNextFridayThe13th(date) {
  ...
  throw new Error('Next Friday the 13th not found');
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (current <= endDate) {
    for (let i = 0; i < countWorkDays && current <= endDate; i += 1) {
      result.push(formateDate(current));
